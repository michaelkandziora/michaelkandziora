...
class FakeClient:
    ...
    def request(self, method, path, data=None):
        if path == '/user':
            return {'login': 'michaelkandziora', 'followers': 3, 'following': 2, 'public_gists': 0}
        if path.startswith('/users/michaelkandziora/starred?'):
            return [{'private': False}, {'private': False}]
        if path == '/graphql':
            if self.graph_error:
...
class UpdateTests(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copytree(ROOT / 'tests/fixtures', self.root)
    ...
    def test_complete_offline_update_twice(self):
        ...
        self.assertEqual(cache['collector_version'], stats.COLLECTOR_VERSION)
        self.assertEqual(cache['metrics']['stars_received']['value'], 9)
        self.assertEqual(cache['metrics']['stars']['value'], 9)
        self.assertEqual(cache['metrics']['starred']['value'], 2)
        self.assertEqual(cache['metrics']['followers']['value'], 3)
    ...
    def test_strict_failure_does_not_write_any_file(self):
        ...
        with self.assertRaises(github.APIError):
            stats.update(self.root, strict=True, client=FakeClient(graph_error=True))
        self.assertEqual(before, {name: (self.root / name).read_bytes() for name in paths})

    def test_partial_failure_marks_last_verified_value(self):
        ...
        cache = json.loads((self.root / '.profile/stats.json').read_text())
        self.assertEqual(cache['metrics']['stars_received']['value'], 11)

    def test_code_scope_change_invalidates_old_values(self):
        config = json.loads((self.root / 'profile.json').read_text())
        config['code']['enabled'] = True
        oldconfig = copy.deepcopy(config)
        oldconfig['code']['repositories'] = ['michaelkandziora/old']
        cache = {'username': 'michaelkandziora', 'settings': {'code': oldconfig['code']},
                 'metrics': {'loc': {'value': 999, 'status': 'ok'}}}
